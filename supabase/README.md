...
Two things live outside `schemas/` because `supabase db diff` cannot see them:

| file | why |
|---|---|
| `migrations/*_storage_buckets.sql` | the diff ignores the `storage` schema entirely |
| `migrations/*_seed_catalogue.sql` | data, not structure — generated by `pnpm db:seed:catalogue` |
...
auth.users
  └── profiles ────────────── body + goal: the inputs to the calorie budget
       ├── user_settings ──── display, notifications, integrations, privacy
       ...
       ├── workouts ───────── discrete sessions, idempotent on external_id
       ├── daily_activity ─── steps and rings, mirrored from the watch
       └── user_achievements  → achievements

foods ──── food_servings      shared catalogue (owner_id null) + private dishes
achievements                  badge definitions, seeded, read-only to clients
```
...
- **Grants are the outer gate.** Where a client must never write —
  `subscriptions`, `achievements`, `user_achievements` — there is no grant at
  all, not merely no policy, so a policy added later by mistake cannot become
  an entitlement.
...
**Custom dishes live in `foods` with a non-null `owner_id`.** A separate
`custom_foods` table would mean a union on every read and a choice of foreign
key on every join. One nullable column costs one clause in one policy — and it
is the seam the calorie scanner will use.
...
**Calorie scanning.** Out of scope for now, but the shape it needs is already
here: `entry_source` has a `camera` value, `food_logs.photo_path` exists, the
private `meal-photos` bucket exists, and a scan can mint a private `foods` row
and log against it without changing anything. `foods.verified` is what tells a
reviewed dish from a guessed one.
...
**HealthKit / Health Connect.** `workouts` and `daily_activity` both carry
`source` and, on workouts, a unique `(user_id, source, external_id)` so a
re-sync is an upsert and running it twice is free.
