...
/**
 * The same names again, for the edge functions.
 *
 * The model that reads a typed meal or a described recipe is asked to pick the
 * drawing for it, so the server needs to know which drawings exist. Edge
 * functions are Deno and outside the pnpm workspace, so they cannot import the
 * registry above; the alternative to writing both files here is a hand-kept
 * second copy, which drifts the first time an icon is renamed and fails
 * silently — the model names an icon nothing can render and the row goes blank.
 *
 * Names only. The server never draws anything.
 */
const EDGE_REGISTRY = join(APP, '..', 'supabase', 'functions', '_shared', 'icons.generated.ts')
...
// Only the two sets a plate of food can be drawn from. `body`, `system` and
// `ui` are exercise and chrome, and offering them to a model asked to
// illustrate a meal is offering it a wrong answer.
const PICKABLE = ['dishes', 'food']
...
${PICKABLE.map((set) => `  ${set}: [\n${(manifest[set] ?? []).map((n) => `    '${n}',`).join('\n')}\n  ],`).join('\n')}
} as const
`,
)
...
console.log(
  `\n${total} icons across ${sets.length} sets, ${(bytes / 1024 / 1024).toFixed(1)} MB` +
    ...
    `\nwrote ${EDGE_REGISTRY.replace(APP, '.')}`,
)
