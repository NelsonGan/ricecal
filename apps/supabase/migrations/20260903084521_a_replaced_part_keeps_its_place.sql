...
DROP FUNCTION
  public.add_ingredient(p_food_log_id uuid, p_name text, p_kcal numeric, p_carbs_g numeric, p_protein_g numeric, p_fat_g numeric, p_quantity numeric, p_grams numeric, p_food_id
  uuid, p_serving_id text, p_serving_label text);
...
COMMENT ON FUNCTION public.add_ingredient(uuid,text,numeric,numeric,numeric,numeric,numeric,numeric,uuid,text,text,smallint) IS 'Put one food on a logged entry''s plate. Seeds the entry itself as the first part when it has no breakdown yet, so the totals the view sums stay the entry''s own. Owner-checked.';

GRANT ALL ON FUNCTION public.add_ingredient(uuid, text, numeric, numeric, numeric, numeric, numeric, numeric, uuid, text, text, smallint) TO authenticated;

GRANT ALL ON FUNCTION public.add_ingredient(uuid, text, numeric, numeric, numeric, numeric, numeric, numeric, uuid, text, text, smallint) TO service_role;-- Hand-written: `db diff` emits the grants a function ends up with and not the
...
REVOKE EXECUTE ON FUNCTION public.add_ingredient(
  uuid, text, numeric, numeric, numeric, numeric, numeric, numeric, uuid, text, text, smallint
) FROM public, anon;
