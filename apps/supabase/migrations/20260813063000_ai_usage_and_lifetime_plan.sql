-- AI inference tracking, and the monthly ceiling on it.
--
-- Adds `ai_usage` plus the three functions that read and move it, and adds
-- `lifetime` to `subscription_plan` for the one-off purchase the stores now
-- sell alongside the two subscriptions.
--
-- Everything below the enum is copied VERBATIM out of
-- `schemas/17_ai_usage.sql`, comments and all. Postgres stores `prosrc`
...
-- Adding a value to an enum is allowed inside a transaction on PG12+ so long
-- as nothing in the same transaction USES the new value. Nothing here does:
-- the column already exists and no row is written.
