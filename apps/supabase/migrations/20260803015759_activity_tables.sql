...
-- Mirrors schemas/01_enums.sql, schemas/41_activity.sql and the Activity
-- section of schemas/11_user_settings.sql. Applied to the remote project
-- directly (there was no local stack running), then written here so that
-- `db:reset` and the CI migrations job reach the same schema.
...
-- The one movement goal nobody's watch supplies. See 11_user_settings.sql.
alter table public.user_settings
  add column step_goal integer not null default 8000
    check (step_goal between 1000 and 50000),
  add column activity_extends_budget boolean not null default true;
