...
-- THREE VIEWS COME DOWN, not the one `db diff` found.
--
-- Postgres will not retype a column a view reads, and the tool followed
-- `quantity` only into the view that selects it by name. `food_log_details`
-- sums the same column, and `daily_nutrition` sums that in turn, so the whole
-- chain has to be rebuilt around the ALTER. Innermost last on the way down and
-- first on the way up.
--
-- The bodies below are the ones that were dropped, verbatim: they are not being
-- changed here, only carried across the type change.
DROP VIEW public.daily_nutrition;

DROP VIEW public.food_log_details;

DROP VIEW public.food_log_ingredient_details;

ALTER TABLE public.food_log_ingredients
  ALTER COLUMN quantity TYPE numeric(8,4) USING quantity::numeric(8,4);

ALTER TABLE public.food_log_ingredients
  ALTER COLUMN quantity SET DEFAULT 1;

CREATE VIEW public.food_log_ingredient_details WITH (security_invoker=on) AS SELECT id,
    food_log_id,
    food_id,
    "position",
    COALESCE(display_label, item_name) AS name,
    quantity,
    serving_label,
    (round((((base_kcal)::numeric * serving_factor) * quantity)))::integer AS kcal,
    round(((base_carbs_g * serving_factor) * quantity), 1) AS carbs_g,
    round(((base_protein_g * serving_factor) * quantity), 1) AS protein_g,
    round(((base_fat_g * serving_factor) * quantity), 1) AS fat_g,
    round((grams * quantity), 1) AS grams
...
-- No SELECT for service_role, which is what this view already had. It is read by
-- the app as the signed-in user and by nothing on the server.
