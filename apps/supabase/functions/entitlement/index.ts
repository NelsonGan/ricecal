...
//
// `subscriptions` is a cache of a webhook and `reconcileEntitlement` refills it
// on a miss, but only from inside a Pro-gated request. The scans-left line and
// the plan on the Me tab are read straight out of Postgres, so an account whose
// webhook was lost sees "3 scans left today" until it presses a Pro button.
//
// It grants nothing on the caller's say-so: the body is empty, the account is
// resolved from a verified JWT, and the answer comes from RevenueCat. The worst a
// caller can do by hammering this is ask RevenueCat about themselves.
//
